...
function Modal({ open, onClose, title, children, maxWidth = 'max-w-md' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className={`relative bg-white rounded-3xl shadow-2xl w-full ${maxWidth} animate-fade-up overflow-hidden`}>
...
export default function RideTable({ rides, loading, onDelete, onEdit }) {
  ...
  const inputCls =
    'w-full px-4 py-3 rounded-xl border border-border bg-slate-50/50 text-sm text-primary focus:outline-none focus:ring-4 focus:ring-secondary/10 focus:border-secondary transition-all'
            ...
            <thead>
              <tr className="border-b border-border/60">
                {['Platform', 'Pickup', 'Dropoff', 'Fare', 'Date', 'Actions'].map(col => (
                  <th
                    key={col}
                    className="text-left text-[10px] font-bold text-muted uppercase tracking-[0.1em] px-4 py-3 first:pl-2"
                  >
                    {col}
                  </th>
                ))}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="font-bold text-primary mt-1">No rides found</p>
                ...
                rides.map(ride => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <p className="text-sm font-semibold text-primary truncate max-w-[160px]">{ride.pickup}</p>
                    </td>
                    <td className="px-4 py-4">
                      <p className="text-sm font-semibold text-primary truncate max-w-[160px]">{ride.dropoff}</p>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="p-2 rounded-xl hover:bg-white ring-1 ring-transparent hover:ring-border/50 hover:shadow-sm transition-all"
                          title="View"
                        >
                          <Eye className="w-4 h-4 text-muted hover:text-secondary transition-colors" />
                        </button>
                        <button
                          type="button"
                          ...
                          title="Edit"
                        >
                          <Pencil className="w-4 h-4 text-muted hover:text-primary transition-colors" />
                        </button>
                        <button
                          type="button"
                          onClick={() => setDeleteId(ride.id)}
                          className="p-2 rounded-xl hover:bg-red-50 ring-1 ring-transparent hover:ring-red-100 transition-all"
                          title="Delete"
                        >
                          <Trash2 className="w-4 h-4 text-muted hover:text-red-500 transition-colors" />
                        </button>
                      </div>
                    </td>
                  </tr>
        ...
        {viewRide && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-4 rounded-2xl bg-slate-50 border border-border/40">
              ...
              <div>
                <p className="font-bold text-primary">{viewRide.platform}</p>
                <p className="text-xs text-muted mt-0.5">{getRideDisplayDate(viewRide)}</p>
              </div>
              <span className="ml-auto text-2xl font-extrabold text-primary">₹{viewRide.fare}</span>
            </div>

            <div className="space-y-2.5">
              <div className="flex items-start gap-3 p-3.5 rounded-xl bg-emerald-50/60 border border-emerald-100/60">
                <MapPin className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />
                <div>
                  <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider">Pickup</p>
                  <p className="text-sm font-semibold text-primary mt-0.5">{viewRide.pickup}</p>
                </div>
              </div>
              <div className="flex items-center justify-center">
                <div className="w-px h-4 bg-border/60" />
              </div>
              <div className="flex items-start gap-3 p-3.5 rounded-xl bg-blue-50/60 border border-blue-100/60">
                <MapPin className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" />
                <div>
                  <p className="text-[10px] font-bold text-blue-600 uppercase tracking-wider">Dropoff</p>
                  <p className="text-sm font-semibold text-primary mt-0.5">{viewRide.dropoff}</p>
                </div>
              </div>
            </div>
      ...
      <Modal open={!!editRide} onClose={() => setEditRide(null)} title="Edit Ride">
        {editRide && (
          <div className="space-y-4">
            ...
            <div>
              <p className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Platform</p>
            ...
            <div>
              <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-1.5">Pickup</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-1.5">Dropoff</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-1.5">Fare (₹)</label>
              <input
                type="number"
                min="0"
                step="0.01"
