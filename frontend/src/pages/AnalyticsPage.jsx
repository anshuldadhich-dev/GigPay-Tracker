...
function SkeletonCard() {
  return (
    <div className="rounded-[22px] p-6 bg-card border border-border/40 shadow-card animate-pulse">
...
function KpiCard({ label, value, sub, icon: Icon, color, bg, border }) {
  return (
    <div className={`relative rounded-[22px] p-6 bg-white border ${border} shadow-card overflow-hidden group card-premium`}>
      ...
      <p className="text-[13px] text-muted font-semibold mt-1">{label}</p>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="text-muted text-xs font-medium">₹{fmt(achieved)} / ₹{fmt(goal)}</span>
      </div>
...
function DailyTooltip({ active, payload, label }) {
  ...
  return (
    <div className="glass rounded-2xl px-4 py-3 shadow-soft border border-white/60 text-sm min-w-[140px]">
      ...
      <p className="text-secondary font-extrabold text-base">₹{fmt(payload[0]?.value || 0)}</p>
      {payload[0]?.payload?.rides && (
        <p className="text-muted text-xs mt-0.5">{payload[0].payload.rides} rides</p>
      )}
    </div>
  )
}
...
function DowTooltip({ active, payload, label }) {
  ...
  return (
    <div className="glass rounded-2xl px-4 py-3 shadow-soft border border-white/60 text-sm min-w-[140px]">
      <p className="font-bold text-primary text-xs mb-1">{label}</p>
      <p className="text-secondary font-extrabold text-base">₹{fmt(d?.earnings || 0)}</p>
      <p className="text-muted text-xs mt-0.5">{d?.rides} rides · avg ₹{fmt(d?.avgEarnings || 0)}</p>
    </div>
  )
}
...
function MonthlyTooltip({ active, payload, label }) {
  ...
  return (
    <div className="glass rounded-2xl px-4 py-3 shadow-soft border border-white/60 text-sm min-w-[140px]">
      <p className="font-bold text-primary text-xs mb-1">{label} {d?.year}</p>
      <p className="text-secondary font-extrabold text-base">₹{fmt(d?.earnings || 0)}</p>
      <p className="text-muted text-xs mt-0.5">{d?.rides} rides</p>
    </div>
  )
}
...
function PieTooltip({ active, payload }) {
  ...
  return (
    <div className="glass rounded-2xl px-4 py-3 shadow-soft border border-white/60 text-sm">
      <p className="font-bold text-primary text-xs">{d.name}</p>
      <p className="text-secondary font-extrabold text-base mt-1">₹{fmt(d.value)}</p>
      <p className="text-muted text-xs">{d.payload.percentage}% · {d.payload.rides} rides</p>
    </div>
  )
}
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6 pb-20 max-w-[1440px] mx-auto">
      ...
      {/* Error */}
      {error && (
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-red-50 border border-red-100 text-red-600 text-sm font-medium animate-scale-in">
        ...
        <div className="xl:col-span-2 animate-fade-up" style={{ animationDelay: '200ms' }}>
          <Card className="h-full" padding="lg">
            <div className="mb-5">
              ...
              <h3 className="text-lg font-extrabold text-primary tracking-tight mt-0.5">Daily Earnings</h3>
            </div>
            {loading ? (
        ...
        <div className="animate-fade-up" style={{ animationDelay: '260ms' }}>
          <Card className="h-full" padding="lg">
            <div className="mb-5">
              ...
              <h3 className="text-lg font-extrabold text-primary tracking-tight mt-0.5">Progress</h3>
              <p className="text-xs text-muted mt-1">Always shows today / this week / this month</p>
            </div>
            {loading ? (
              ...
              <div className="space-y-6">
                ...
                <GoalBar
                  ...
                  color="bg-gradient-to-r from-accent to-accent-dark"
                />
                <div className="pt-2 border-t border-border/40 flex items-center gap-2 text-xs text-muted">
                  <Target className="w-3.5 h-3.5 text-secondary shrink-0" />
                  <span>Change goals in <span className="font-semibold text-primary">Settings</span></span>
                </div>
              </div>
            )}
          </Card>
        ...
        <div className="animate-fade-up" style={{ animationDelay: '300ms' }}>
          <Card className="h-full" padding="lg">
            <div className="mb-5">
              ...
              <h3 className="text-lg font-extrabold text-primary tracking-tight mt-0.5">Platform Earnings</h3>
            </div>
            {loading ? (
                    ...
                    <div key={p.platform} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-semibold text-primary truncate">{p.platform}</span>
                          <span className="text-sm font-extrabold text-primary shrink-0">₹{fmt(p.earnings)}</span>
                        </div>
                        <div className="flex items-center justify-between gap-2 mt-0.5">
                          ...
                          <span className="text-xs text-muted shrink-0">{p.rides} rides</span>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <div className="animate-fade-up" style={{ animationDelay: '360ms' }}>
          <Card className="h-full" padding="lg">
            <div className="mb-5">
              ...
              <h3 className="text-lg font-extrabold text-primary tracking-tight mt-0.5">Best Days to Drive</h3>
            </div>
            {loading ? (
              <div className="h-56 skeleton rounded-2xl" />
        ...
        <Card padding="lg">
          <div className="flex items-start justify-between mb-5">
            <div>
              ...
              <h3 className="text-lg font-extrabold text-primary tracking-tight mt-0.5">Monthly Earnings Trend</h3>
              <p className="text-xs text-muted mt-1">Last 12 months — all rides included</p>
            </div>
            {monthly.length > 0 && (
      ...
      <div className="animate-fade-up" style={{ animationDelay: '480ms' }}>
        <Card padding="lg">
          <div className="mb-5">
            ...
            <h3 className="text-lg font-extrabold text-primary tracking-tight mt-0.5">Most Frequent Routes</h3>
            <p className="text-xs text-muted mt-1">Top 8 routes in the selected period</p>
          </div>
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border/40">
                  {routes.map((r, i) => (
                    <tr key={`${r.pickup}-${r.dropoff}-${i}`} className="hover:bg-slate-50 transition-colors animate-fade-up" style={{ animationDelay: `${i * 40}ms` }}>
                      <td className="py-3.5 pr-3">
                        <span className={`inline-flex items-center justify-center w-6 h-6 rounded-lg text-xs font-bold ${i === 0 ? 'bg-yellow-50 text-yellow-600' : i === 1 ? 'bg-slate-100 text-slate-500' : i === 2 ? 'bg-orange-50 text-orange-500' : 'bg-slate-50 text-muted'}`}>
                          {i + 1}
                        </span>
                      </td>
                      <td className="py-3.5 pr-4">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="font-medium text-primary truncate max-w-[120px] sm:max-w-[200px]">{r.pickup}</span>
                          <Zap className="w-3 h-3 text-slate-300 shrink-0" />
                          <span className="text-muted truncate max-w-[120px] sm:max-w-[200px]">{r.dropoff}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-3.5 text-right font-extrabold text-primary">₹{fmt(r.totalEarnings)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
